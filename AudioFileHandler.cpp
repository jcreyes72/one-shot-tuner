...
void processAudio(const AudioFileInfo& fileInfo) {
    ...
    // Determine the overall note based on occurrences and magnitudes
    std::string dominantNote;
    double maxWeightedMagnitude = 0.0;

    for (const auto& [note, count] : noteCounts) {
        double averageMagnitude = noteMagnitudes[note] / count; // Calculate the average magnitude
        if (averageMagnitude > maxWeightedMagnitude) {
            maxWeightedMagnitude = averageMagnitude;
            dominantNote = note;
        }
    }

    std::cout << "Overall note: " << dominantNote << ", Average Magnitude: " << maxWeightedMagnitude << "\n";
